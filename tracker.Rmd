...
ns <- xml_ns(runk)
xml_find_all(runk, ".//d1:trkpt", ns = ns)
...
Connect platform actively discourages users from this, with the only easy 
solution being a web app that wants your credentials to impersonate you. 😞:(
...
runs <- dir("./data/garmin", "*.TCX", full.names = TRUE) %>% 
  purrr::map_df(read_data)
runs <- as_data_frame(runs)
```
...
map_df(runs_trackr, ~ data_frame(start = range(index(.))[1], stop = range(index(.))[2]), .id = NULL) %>% mutate(start = as.Date(start)) %>% select(start) -> agg_dates
...
ggplot(high_hr, aes(x = values, y = lengths)) + 
  ...
  labs(title = "High Heart Rate Occurances", x = "Heart Rate", 
       ...
       caption = paste("Maximum HR assumed to be", zones$max)) + 
